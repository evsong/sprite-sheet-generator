...
export function AiProgressToast() {
  ...
  const done = progress.completed === progress.total && !progress.error;
    ...
    <div className="fixed bottom-16 right-4 z-[200] w-64 bg-[#0D0D0D] border border-[#1E1E1E] rounded-lg p-3 shadow-xl">
      <div className="flex items-center gap-2 mb-2">
        {progress.error ? (
          ...
          <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2">
            <path d="M21 12a9 9 0 11-6.219-8.56" />
          </svg>
        )}
        <span className="text-[11px] font-[family-name:var(--font-mono)] text-white truncate flex-1">
          {progress.error ? "Generation failed" : done ? "Done" : "Generating..."}
        </span>
        <span className="text-[10px] font-[family-name:var(--font-mono)] text-[#666]">
      ...
      <div className="h-1 bg-[#1A1A1A] rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-300 ${
            progress.error ? "bg-red-500" : done ? "bg-[#22C55E]" : "bg-[#F59E0B]"
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
